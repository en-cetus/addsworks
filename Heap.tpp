...
public:
    ...
    void remove(T value)
    {
        ...
        typename heap::iterator itBegin = this->tree.begin();
        ++itBegin; // index = 1
        typename heap::iterator it = std::find(itBegin, this->tree.end(), value);
        //Not found
        if (it == this->tree.end()) return;
        //Remove element
        this->tree.erase(it);
        //Rebuild the heap
        if (!this->isHeapEmpty()) {
    ...
    T getMin()
    {
        ...
        return this->tree[1]; // Stack Top (Minimum)
    }
};
